...
const VoyagerCard = ({ voyager }: VoyagerCardProps) => {
  ...
  return (
    <div className="mx-8 relative cursor-pointer">
      <div
        className="bg-nexusDarkBg rounded-xl mb-6 p-4 border border-nexusGreen relative sm:hover:scale-110 sm:hover:z-10 sm:hover:shadow-voyagerCard voyager-card-wrapper"
      ...
      >
        ...
        <p
          className={`text-astromapGrayText rounded-lg ${
            isHover ? "sm:block" : ""
          }`}
        >
          {voyager.description}
        ...
        {!isHover && (
          <div className="absolute right-0 bottom-0 pb-2 pr-3">
